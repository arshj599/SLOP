...
export function logUsage(event) { const db = readDb(); db.usage_events.push({ ...event, metadata: event.metadata || {}, timestamp: event.timestamp || new Date().toISOString() }); writeDb(db); }
